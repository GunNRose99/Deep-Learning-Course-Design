...
def train_cnn_lstm(model_name):
    ...
    for epoch in range(40):
        ...
        if val_loss < min_loss:
            min_loss = val_loss
            torch.save(model.state_dict(), f"layers4/{model_name}_best.pth")
...
def plot_acc_loss(train_accs, train_losses, val_accs, val_losses, model_name):
    ...
    plt.ylabel("loss")
    plt.legend()
    plt.savefig(f"layers_image/{model_name}_epoch_acc_loss.jpg")


if __name__ == '__main__':
    train_cnn_lstm(model_name="CnnLstmModel_1")
    train_cnn_lstm(model_name="CnnLstmModel_2")
    train_cnn_lstm(model_name="CnnLstmModel_3")
    train_cnn_lstm(model_name="CnnLstmModel_4")